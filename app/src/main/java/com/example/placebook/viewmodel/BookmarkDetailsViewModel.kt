...
class BookmarkDetailsViewModel(application: Application) : AndroidViewModel(application) {
    ...
    // This is a helper method that converts from the database bookmark to the View bookmark
    private fun mapBookmarksToMarkerView(bookmarkId: Long) {
    ...
    fun getBookmark(bookmarkId: Long): LiveData<BookmarkDetailsView>? {
        if (bookmarkDetailsView == null) {
            mapBookmarksToMarkerView(bookmarkId)
        }
        return bookmarkDetailsView
    }

    private fun bookmarkViewToBookmark(bookmarkView: BookmarkDetailsView): Bookmark? {
        val bookmark: Bookmark? = bookmarkView.id?.let {
